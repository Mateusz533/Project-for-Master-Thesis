...
class HeatingPlate : public SystemElement, public TemperatureSensor
{
  ...
  private:
    ...
    const short unsigned int PIN_HEAT_SUPPLY_;
...
void HeatingPlate::regulateHeatingPower()
{
  ...
  temperature_derivative_ = 1.0 * (real_temperature_ - set_temperature_ - temperature_deviation_)
                            * 1000 / TEMPERATURE_ESTIMATION_PERIOD;                                 // zamiana ms w s
...
int HeatingPlate::calculateRegulatedHeatingPower()
{
  ...
  if (!active_regulation_)
  {
    ...
    for (int i = 0; i < CONVERSION_ARRAYS_SIZE_; i++)
    {
      ...
      {
        heating_power = round(HEATING_POWER_VALUES_[i - 1] + 1.0  * (set_temperature_ - TEMPERATURE_VALUES_[i - 1])
                              * (HEATING_POWER_VALUES_[i] - HEATING_POWER_VALUES_[i - 1]) / (TEMPERATURE_VALUES_[i] - TEMPERATURE_VALUES_[i - 1]));
        break;
      }
      else
  ...
  int power_deviation = round(- DIFFERENTIAL_REGULATION_COEFFICIENT * temperature_derivative_
                              - PROPORCIONAL_REGULATION_COEFFICIENT * temperature_deviation_
